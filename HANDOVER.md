...
- **Lane-vanish safety (`_handle_lane_merge`):** traffic has a forward `drive_speed`, so a car's *world position* drifts forward as it's overtaken and can cross into a section with a different lane layout. A car whose lane no longer exists there (e.g. the 3-lane middle at X=0 on a 2-lane road) would be an undodgeable block, so while still **ahead** of the player it **disables its collision** (`collision_layer = 0`, harmless even if it overlaps a lane) and **steers off to the nearest shoulder** (`MERGE_OFF_SPEED`), then frees once off the asphalt. `_lane_valid_at(bx, cfg)` (tolerance `LANE_MATCH_TOL`) is the check. Pedestrians are exempt (`drive_speed = 0`, no drift).
